...
const MentorApproval = () => {
  ...
  const handleApprove = (voucherId: string) => {
    console.log(`Voucher ${voucherId} approved by mentor.`);
    // In a real application, this would update the voucher's status in a backend.
    // For now, it's a placeholder.
  };

  const handleReject = (voucherId: string) => {
    console.log(`Voucher ${voucherId} rejected by mentor.`);
    // In a real application, this would update the voucher's status in a backend.
    // For now, it's a placeholder.
  };
      ...
      {submittedVouchers.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
          {submittedVouchers.map((item) => (
            <Card key={item.id} className="shadow-lg border-purple-300">
              <CardHeader className="bg-purple-100 rounded-t-lg p-4">
                <CardTitle className="text-xl font-bold text-purple-800">{item.voucherHeading}</CardTitle>
              ...
              <CardContent className="p-4 space-y-2 text-gray-700">
                ...
                <div className="flex justify-end space-x-2 mt-4">
                  <Button variant="outline" className="text-green-600 border-green-600 hover:bg-green-50" onClick={() => handleApprove(item.id)}>
                    অ্যাপ্রুভ করুন
                  </Button>
